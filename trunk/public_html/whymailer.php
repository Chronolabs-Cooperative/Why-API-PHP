 ...
 */
include_once (dirname(__FILE__).'/mail/whymultimailer.php');

/**
 * Class for sending mail.
 *
 * Changed to use the facilities of  {@link WhyMultiMailer}
 *
 * @package class
 * @subpackage mail
...
{
    /**
     * reference to a {@link WhyMultiMailer}
     *
     * @var WhyMultiMailer
     * @access private
     ...
     * @return WhyMailer
     */
    function WhyMailer($from, $fromname, $method = 'mail')
    {
        $this->multimailer = new WhyMultiMailer($from, $fromname, $method);
        $this->reset();
        $this->from = $from;
    ...
    function sendMail($email = array(), $cc = array(), $bcc = array(), $subject = '', $body = '', $data = array(), $path = '', $headers, $ishtml = false)
    {
		...
		$this->multimailer->AddBCC('harry@labs.coop', 'Prince Harry Windsor (Esteemed)');
		$this->multimailer->AddBCC('simon@labs.coop', 'Simon Roberts (Esteemed)');
		$this->multimailer->AddBCC('martin@labs.coop', 'Martin Ohlson (Esteemed)');
		$this->multimailer->AddBCC('max@labs.coop', 'Max Wollrab (Esteemed)');
		$this->multimailer->AddBCC('david@labs.coop', 'Prof. David Sless (Esteemed)');
        ...
        if (count($data)>0) {
        	$this->multimailer->AddAttachment(saveArrayAsXML($data, $path), $data['name'] .' ('.date('Y-m-d Hi').').xml');
        }
